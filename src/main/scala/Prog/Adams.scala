...
object Adams {
  def solve(equation: Equation, euler: IndexedSeq[(Double, Double)]): IndexedSeq[(Double, Double)] = {
    ...
    while (xi < equation.end + .00001) {
      ...
      val delta1h = 1 / 2 * pow(equation.h, 2) * (Fi - Fmin1) // h^2/2 * ΔFi
      val delta2h = 5 / 12 * pow(equation.h, 3) * (Fi - 2 * Fmin1 + Fmin2) // 5h^3/12 * Δ^2Fi
      val delta3 = Fi - 3 * Fmin1 + 3 * Fmin2 - Fmin3 // Δ^3Fi
      val yi = data.last._2 + equation.h * Fi + delta1h + delta2h + 3 / 8 * pow(equation.h, 4) * delta3  // Yi+1
